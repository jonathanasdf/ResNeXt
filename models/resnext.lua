...
local function createModel(opt)
   ...
   if opt.dataset == 'imagenet' then
      ...
      local block1 = layer(block, 64, def[1])
      p, g = block1:getParameters()
      print('Block 1: ' .. g:nElement() .. ' parameters.')
      model:add(block1)
      local block2 = layer(block, 128, def[2], 2)
      p, g = block2:getParameters()
      print('Block 2: ' .. g:nElement() .. ' parameters.')
      model:add(block2)
      local block3 = layer(block, 256, def[3], 2)
      p, g = block3:getParameters()
      print('Block 3: ' .. g:nElement() .. ' parameters.')
      model:add(block3)
      local block4 = layer(block, 512, def[4], 2)
      p, g = block4:getParameters()
      print('Block 4: ' .. g:nElement() .. ' parameters.')
      model:add(block4)
      model:add(Avg(7, 7, 1, 1))
      model:add(nn.View(nFeatures):setNumInputDims(3))
      model:add(nn.Linear(nFeatures, 1000))
      p, g = model:getParameters()
      print('Total: ' .. g:nElement() .. ' parameters.')
